...
public enum AirportWeatherService {
    ...
    public Airport findAirport(String iataCode) {
        return getAirports().stream().filter(ap -> ap.getIata().
                equals(iataCode)).findFirst().
                orElse(null);
    }
     ...
     * @param dp  datapoint object holding pointType data
     *
     * @throws WeatherException if the update can not be completed NoSuchElementException if
     *             airport founded
     */
